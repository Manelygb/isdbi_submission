...
def create_pco():
    ...
    pco = {
        ...
        "processing_log": [{
            ...
            "message": "PCO initialized for testing first 5 agents.",
            "status": "system"
        }],
...
def save_pco_output(pco, filename=None):
    ...
    if filename is None:
        ...
        filename = f"first_five_agents_test_{timestamp}.json"
...
def run_first_five_agents():
    """Run the first 5 agents in sequence"""
    # Create initial PCO
    pco = create_pco()
    ...
    if pco.get("contract_draft"):
        contract_length = len(str(pco['contract_draft'].get('content', '')))
        print(f"Contract Draft: {contract_length} characters")
...
if __name__ == "__main__":
    pco, output_path = run_first_five_agents()
