...
def create_pco():
    ...
    pco = {
        ...
        "processing_log": [{
            ...
            "message": "PCO initialized for testing all eight agents.",
            "status": "system"
        }],
...
def save_pco_output(pco, filename=None):
    ...
    if filename is None:
        ...
        filename = f"first_eight_agents_test_{timestamp}.json"
...
def run_first_eight_agents():
    """Run all 8 agents in sequence including the final decision agent"""
    # Create initial PCO
    pco = create_pco()
    ...
    agents = [
        ...
        FinalDecisionAgent()  # Add final decision agent as the 8th agent
    ]
    ...
    # Print final decision report details
    if pco.get("final_decision_report"):
        print("\n--- Final Decision Report ---")
        print(f"Overall Assessment: {pco['final_decision_report'].get('overall_assessment')}")
        print(f"Confidence Level: {pco['final_decision_report'].get('confidence_level')}")
        
        # Print executive summary preview
        exec_summary = pco['final_decision_report'].get('executive_summary', '')
        if exec_summary:
            preview = exec_summary[:200] + "..." if len(exec_summary) > 200 else exec_summary
            print(f"\nExecutive Summary Preview: {preview}")
        
        # Print key findings
        key_findings = pco['final_decision_report'].get('key_findings', [])
        if key_findings:
            print(f"\nKey Findings: {len(key_findings)} findings")
            for i, finding in enumerate(key_findings[:3]):  # Print first 3 findings
                print(f"  {i+1}. {finding}")
...
if __name__ == "__main__":
    pco, output_path = run_first_eight_agents()
